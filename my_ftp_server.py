...
def process(req):
    ...
    if req == 'pwd':
        ...
        newpath = '/'.join([dirname, re.search(r"<([A-Za-z0-9_]+)>", req).group(1)])
        ...
        if not os.path.exists('/'.join([dirname, file_name])):
            text_file = open('/'.join([dirname, file_name]), "w")
        ...
        mydir = '/'.join([dirname, re.search(r"<([A-Za-z0-9_]+)>", req).group(1)])
        print(mydir)
        ...
        path = '/'.join([dirname, re.search(r"<([A-Za-z0-9_]+)>", req).group(1)])
